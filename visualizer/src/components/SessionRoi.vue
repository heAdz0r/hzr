...
<template>
  <article class="session-roi" aria-labelledby="session-roi-title">
    ...
    <!-- 0.9.1: the priced figure is the point of this card; it was hidden inside a
         collapsed disclosure and, whenever host delivery was unconfirmed, not
         computed at all. -->
    <div v-if="roi.raw_public_estimate" class="session-roi-value">
      <span>Potential public-list savings · {{ roi.selected_model || "no model" }} · {{ roi.raw_public_estimate.pricing_basis }} rate</span>
      <strong>{{ formatEconomicAmount(roi.raw_public_estimate.currency, roi.raw_public_estimate.savings_microunits) }}</strong>
      <small>{{ formatCount(roi.raw_public_estimate.avoided_input_tokens_estimated) }} avoided tokens priced as future model input. {{ roi.raw_public_estimate.delivery_qualifier ?? roi.raw_public_estimate.disclaimer }}</small>
    </div>
    <div v-else-if="roi.session_hash" class="session-roi-unavailable">
    ...
    <details class="detail-disclosure">
      ...
      <p v-if="roi.raw_public_estimate" class="session-roi-disclaimer">{{ roi.raw_public_estimate.disclaimer }}</p>
      <dl class="session-roi-evidence">
        ...
        <div><dt>Catalog</dt><dd><code>{{ roi.raw_public_estimate?.catalog_identity ?? roi.catalog_identity ?? "Not loaded" }}</code><small v-if="roi.raw_public_estimate"> · entry {{ roi.raw_public_estimate.entry_version }}</small></dd></div>
