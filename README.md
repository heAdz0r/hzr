...
<p align="center">
<a href="Cargo.toml"><img alt="Version 0.11.2" src="https://img.shields.io/badge/version-0.11.2-e64a19"></a>
...
HZR 0.11.2 ships self-contained native bundles for Linux x86_64/ARM64 and macOS
...
curl --proto '=https' --tlsv1.2 -fL \
  https://raw.githubusercontent.com/heAdz0r/hzr/v0.11.2/install.sh \
...
task, and what that work reportedly cost. Monitoring creates no task, starts no agent,
...
and the confirmed Apache-2.0 licensing provenance are recorded in
...
publish_command_summaries = false # 0.9.1: hide `cargo test --locked`-style command summaries
...
| durable memory | `hzr_memory_recall`, `hzr_memory_get`, `hzr_memory_store`, `hzr_memory_update`, `hzr_memory_forget`, `hzr_memory_prune` |
...
Since 0.8.3 the first session on a new HZR version schedules this pass once, detached from the
...
directory is also absent is retained in case the volume is unmounted.
...
contain no credentials. The worker ends every run with a report (status, changed paths,
verification commands with exit codes, risks) and one summary line with its tokens; its turn
quota is extended while it makes progress, and a run without a report exits 2 with a report
HZR writes from what the worker actually did.
...
Workers run under the repository's own `AGENTS.md` and `CLAUDE.md`. Rules larger than the
24 KiB preload budget are preloaded head-first and read on demand, so delegation works in
every repository rather than only in small ones. `hzr doctor` reports both the worker
credential and that split.
