...
fn open_journal_lock(path: &Path) -> Result<File, ExecError> {
    open_private_lock(&journal_lock_path(path)) // 0.8.3: one path builder shared with removal
}

fn open_drain_lock(handle: &ForkAccountingHandle) -> Result<File, ExecError> {
    open_private_lock(&drain_lock_path(handle)) // 0.8.3: one path builder shared with removal
}

// 0.8.3: lock paths are built in one place so `remove_accounting_locks` cannot drift from the
...
/// 0.8.3: remove the lock files a correlation leaves behind.
///
...
/// 0.8.3: the correlation a lock file in the fork directory belongs to, or `None` for any other
...
mod tests {
    ...
    // 0.8.3: locks are removed only on request and their absence is not an error.
    #[test]
