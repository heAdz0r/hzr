...
fn sibling_engine_directory(executable: &Path) -> Option<PathBuf> {
    ...
    // `versions/v0.4.2-<platform>/engines`, so after the next upgrade a new `hzr` would
...
mod tests {
    ...
    fn test_engine_directory_uses_upgrade_stable_current_path() {
        ...
        let release = versioned_bundle(root, "v0.4.2-darwin-arm64");
    ...
    fn test_upgrade_switches_engines_through_current() {
        let directory = tempdir().expect("temporary directory");
        let root = directory.path();
        let old = versioned_bundle(root, "v0.4.2-darwin-arm64");
        ...
        let new = versioned_bundle(root, "v0.4.2-darwin-arm64");
