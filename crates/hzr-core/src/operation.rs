...
/// Command prefixes that mean "HZR handed this straight to the shell".
///
/// Both the Rust classifier and the SQL predicate are generated from this list, so the
...
fn strip_bypass_prefix(words: &[String]) -> (OperationRoute, &[String]) {
    for prefix in BYPASS_FALLBACK_PREFIXES {
        ...
        if starts_with_words(words, prefix, true) {
            return (OperationRoute::Bypassed, &words[words.len().min(parts)..]);
        }
    }
    // Longest prefix first so `rtk proxy` is not shadowed by a bare `proxy` check.
    let mut ordered = BYPASS_PREFIXES;
    ordered.sort_unstable_by_key(|prefix| std::cmp::Reverse(prefix.len()));
    for prefix in ordered {
        let parts = prefix.split(' ').count();
        if starts_with_words(words, prefix, false) {
            return (OperationRoute::Bypassed, &words[words.len().min(parts)..]);
        }
    }
    // `rtk -- raw <cmd>` and `hzr -- proxy <cmd>` keep the separator in the middle.
    if words.len() >= 3
        && matches!(words[0].as_str(), "rtk" | "hzr")
        && words[1] == "--"
        && matches!(words[2].as_str(), "raw" | "proxy")
    {
        return (OperationRoute::Bypassed, &words[3..]);
    }
    (OperationRoute::Optimized, words)
}

fn starts_with_words(words: &[String], prefix: &str, allow_trailing_colon: bool) -> bool {
    let expected = prefix.split(' ').collect::<Vec<_>>();
    ...
    words
        .iter()
        .zip(&expected)
        .all(|(word, expected)| match allow_trailing_colon {
            true => word.trim_end_matches(':') == *expected,
            false => word == expected,
        })
}
