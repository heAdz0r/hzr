...
#[test]
fn install_journal_recovers_after_failure_and_preserves_concurrent_user_edit() {
    ...
    fs::write(workspace.join("AGENTS.md"), "# Original user rules\n").expect("AGENTS fixture");
    ...
    let failed = install_command(&home, &workspace, &config, &prefix)
        .env("HZR_TEST_INSTALL_FAIL_AFTER", "hooks")
        .output()
    ...
    let incomplete: serde_json::Value =
        serde_json::from_slice(&fs::read(&journal_path).expect("incomplete journal"))
            .expect("journal JSON");
    assert_ne!(incomplete["state"], "complete");
    assert_eq!(
    ...
    let mut user_settings: serde_json::Value =
        serde_json::from_slice(&fs::read(&settings).expect("hook settings"))
            .expect("settings JSON");
    user_settings["concurrent_user_key"] = serde_json::json!({"keep": true});
    fs::write(
        &settings,
        serde_json::to_vec_pretty(&user_settings).expect("settings encoding"),
    )
    ...
    .expect("concurrent instruction edit");
    fs::create_dir_all(workspace.join(".codex")).expect("Codex config directory");
    fs::write(
        workspace.join(".codex/config.toml"),
        "model = \"user-choice\"\n\n[mcp_servers.other]\ncommand = \"other\"\n",
    )
    .expect("concurrent project Codex edit");
    ...
    let completed: serde_json::Value =
        serde_json::from_slice(&fs::read(&journal_path).expect("completed journal"))
            .expect("journal JSON");
    assert_eq!(completed["state"], "complete");
    assert_eq!(completed["attempt"], 2);
    let final_settings: serde_json::Value =
        serde_json::from_slice(&fs::read(settings).expect("final settings"))
            .expect("settings JSON");
    assert_eq!(final_settings["concurrent_user_key"]["keep"], true);
    let instructions = fs::read_to_string(workspace.join("AGENTS.md")).expect("instructions");
    assert!(instructions.contains("# Concurrent user rules"));
    assert!(instructions.contains("Keep this exact."));
    let codex = fs::read_to_string(workspace.join(".codex/config.toml")).expect("Codex config");
    assert!(codex.contains("model = \"user-choice\""));
    assert!(codex.contains("[mcp_servers.other]"));
    assert!(codex.contains("[mcp_servers.hzr]"));
    assert!(prefix.join("hzr").is_file());
    assert!(prefix.join("hzrd").is_file());
}
