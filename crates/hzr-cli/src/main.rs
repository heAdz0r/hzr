...
async fn run(cli: Cli) -> Result<ExitCode> {
    ...
    match cli.command {
        ...
        Command::Doctor {
            ...
            // 0.9.9: `--fix` closes stale open daemon-unreachable accounting gaps left by an
            ...
            report.accounting_gap_repair = accounting_gap_repair; // 0.9.9
            ...
            if cli.json {
                ...
                if let Some(count) = report.accounting_gap_repair {
                    output::print_accounting_gap_repair(count)?; // 0.9.9
                }
                print_doctor(&report)?;
            }
...
async fn initialize_if_needed(
    ...
    // 0.8.3: the first session on a new HZR version schedules one detached fleet-wide doctor
    ...
    if json {
        print_json(&serde_json::json!({
            ...
            "dashboard": dashboard,
            "daemon_service": service_report,
            "reference_state": reference_state, // 0.8.3
            "mcp": mcp::lifecycle_metadata(),
        }))?;
    } else if !quiet {
        let stdout = io::stdout();
