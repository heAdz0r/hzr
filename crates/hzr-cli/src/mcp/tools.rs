...
mod tests {
    ...
    fn acceptance_gate_mcp_descriptions_are_unique_and_schema_is_bounded() {
        // Every byte here is loaded into every session that lists tools. The 13-tool typed
        // inventory measured 25,574 bytes on 2026-09-04 before the codec entry was compacted
        // (W12); keep the whole result under 25 KiB so capability coverage cannot silently
        // recreate the former 64 KiB prompt budget.
