...
fn launchd(action: ServiceCommand, home: &Path, binary: &Path) -> Result<ServiceReport> {
    ...
    match action {
        ServiceCommand::Install => {
            // 0.9.6: re-bootstrap only when the definition changed or the job is not loaded.
...
mod tests {
    ...
    fn test_service_definitions_use_stable_current_binary() {
        ...
        let executable = root.join("versions/v0.4.6-test/bin/hzr");
    ...
    fn test_only_versioned_bundle_layout_is_a_production_install() {
        assert!(is_versioned_install(Path::new(
            "/opt/hzr/versions/v0.4.6-test/bin/hzr"
        )));
    ...
    fn test_public_binary_symlink_resolves_back_to_stable_service() {
        let directory = tempdir().expect("temporary directory");
        let root = directory.path();
        let release_bin = root.join("versions/v0.4.6-test/bin");
        ...
        std::os::unix::fs::symlink(root.join("versions/v0.4.6-test"), root.join("current"))
            .expect("current link");
