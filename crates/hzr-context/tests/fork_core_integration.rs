...
case "$1" in
  ...
  rgai)
    ...
    printf '%s\n' '{"query":"managed_fork_hit","path":"src","total_hits":1,"shown_hits":1,"scanned_files":1,"skipped_large":0,"skipped_binary":0,"hits":[{"path":"lib.rs","score":9.5,"matched_lines":1,"snippets":[{"lines":[{"line":1,"text":"pub fn managed_fork_hit() {}"}],"matched_terms":["managed_fork_hit"]}]}]}'
    ;;
  memory)
    if [ "$2" != "plan" ]; then
      exit 66
