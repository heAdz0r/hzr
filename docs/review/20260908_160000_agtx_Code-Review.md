...
The 0.8.602 publication was cancelled before assets were published after GitHub
rejected a job-level runner context in the newly added agtx test environment.
Use literal paths inside the disposable Ubuntu runner instead. The immutable
0.8.602 tag is retained; the corrected release target is 0.8.603. Product hotfix
code is unchanged apart from canonical version synchronization. This correction
is validated by actionlint and a fresh CI/release run.

## Release decision

Publish the corrected default HZR bundle as 0.8.603 after its source and native
