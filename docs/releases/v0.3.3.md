...
## Known gap (historical — as of 0.3.3)

As of this release, `hzr memory` had no `forget` or `prune`. Legacy-imported rows were
quarantined from automatic project recall, so they no longer surfaced as if they described
this repository — but they still occupied the namespace and could not be removed through the
sanctioned control plane. The engine already supported deletion; the HZR path did not exist
yet.

This gap was closed in 0.3.4. On current HZR releases, use `hzr memory forget`,
`hzr memory update`, and `hzr memory prune` (CLI, daemon API, MCP, and managed-agent bridge).
See [v0.3.4](v0.3.4.md) and the root README basic commands.
