...
PATH="${HZR_INSTALLED_BIN}:/usr/bin:/bin" hzr --version | grep -Fx "hzr 0.9.6" >/dev/null
...
for HZR_HOOK_PERMISSION in absent default bypassPermissions; do
  ...
  # 0.9.6: in a prompting mode with no Claude allow rule for the command, the hook stays silent so
...
HZR_UPGRADE_VERSION="0.9.6-upgrade-smoke"
