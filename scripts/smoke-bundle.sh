...
"${HZR_BINARY_ROOT}/hzr" --version | grep -Fx "hzr 0.9.8" >/dev/null
...
"${HZR_NODE_BINARY}" -e '
  const report = JSON.parse(process.argv[1]);
if (report.protocol_version !== 1 || report.hzr_version !== "0.9.8") {
...
if (response.status !== 200 || report.hzr_version !== "0.9.8" ||
