...
for HZR_REQUIRED_FILE in \
  ...
  TRACKED_CHANGES.patch SOURCE_STATUS; do
...
HZR_CHECKSUM_OUTPUT="$(
  ...
  echo "fork-core file checksum verification failed:" >&2
...
HZR_CONTENT_MANIFEST_SHA256="${HZR_ACTUAL_CONTENT_MANIFEST_SHA256}"
HZR_CORE_VERSION="${HZR_EXPECTED_CORE_VERSION}"
HZR_SELECTION="$(hzr_snapshot_toml_string selection)"
...
hzr_snapshot_emit_v2 \
  "${HZR_CORE_ROOT}" \
  "${HZR_SNAPSHOT_ROOT}/FORK_FILES" \
  "${HZR_SNAPSHOT_ROOT}/TRACKED_DELETIONS" \
  "${HZR_SNAPSHOT_ROOT}/TRACKED_CHANGES.patch" \
  "${HZR_SNAPSHOT_ROOT}/SOURCE_STATUS" \
  >"${HZR_VERIFY_TEMP}/SNAPSHOT_V2.actual.tsv"
if ! cmp -s \
  "${HZR_SNAPSHOT_ROOT}/SNAPSHOT_V2.tsv" \
  "${HZR_VERIFY_TEMP}/SNAPSHOT_V2.actual.tsv"; then
  echo "fork-core entry/type/mode/size/content metadata drift detected:" >&2
  diff -u \
    "${HZR_SNAPSHOT_ROOT}/SNAPSHOT_V2.tsv" \
    "${HZR_VERIFY_TEMP}/SNAPSHOT_V2.actual.tsv" | sed -n '1,80p' >&2 || true
  exit 1
fi
...
done <"${HZR_SNAPSHOT_ROOT}/FORK_FILES"

while IFS= read -r HZR_ACTUAL_PATH; do
  HZR_RELATIVE_PATH="${HZR_ACTUAL_PATH#${HZR_CORE_ROOT}/}"
  if grep -Fqx -- "${HZR_RELATIVE_PATH}" "${HZR_SNAPSHOT_ROOT}/FORK_FILES"; then
    continue
  fi
...
if [[ -s "${HZR_VERIFY_TEMP}/unexpected" ]]; then
  echo "unexpected non-ignored files were added to immutable fork-core:" >&2
...
echo "fork-core snapshot v2 ${HZR_EXPECTED_SNAPSHOT_SHA256} verified"
