...
// 0.11.0 (US-015, upstream 361057e/2bdea45/f83be19): one table per engine. The
// shared table read rg's `-r REPLACEMENT` and `-E ENCODING` as booleans (the value
// became the pattern) and grep's boolean `-T` as taking a value.
const GREP_VALUE_FLAGS_SHORT: &[u8] = b"ABCDdfm";
const RG_VALUE_FLAGS_SHORT: &[u8] = b"ABCEMTdfgjmrt";
...
const VALUE_FLAGS_LONG: &[&str] = &[
    ...
    "--context",
    "--context-separator",
    // 0.11.0 (US-015): GNU grep long options that take a separate value.
    "--binary-files",
    "--devices",
    "--directories",
    "--exclude",
...
const DEFAULT_MAX_PER_FILE: usize = 25;
...
fn parse_cluster(engine: Engine, rest: &str) -> ClusterResult {
    let value_flags = engine.value_flags_short(); // 0.11.0 (US-015)
...
impl Engine {
    /// Short flags that consume a value, for this engine's own grammar. (0.11.0, US-015)
    fn value_flags_short(self) -> &'static [u8] {
        match self {
            Engine::Grep => GREP_VALUE_FLAGS_SHORT,
            Engine::Rg => RG_VALUE_FLAGS_SHORT,
        }
    }
...
pub fn run(
    ...
    let per_file = DEFAULT_MAX_PER_FILE;
    ...
    for (idx, (file, entries)) in files.into_iter().enumerate() {
        ...
        match crate::tee::force_tee_tail_hint(&full_block, &grep_slug(idx, file), file_shown + 1) {
    ...
    if skipped_files > 0 {
        let hint = crate::tee::force_tee_tail_hint(&skipped_block, "grep_skipped", 1)
            .map(|h| format!(" {}", h))
            .unwrap_or_default();
...
mod tests {
    ...
    // 0.11.0 (US-015): each engine's own value-taking short flags.
    #[test]
    fn test_engine_specific_value_flags() {
        // rg: -r REPLACEMENT and -E ENCODING take values.
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Rg, &["-r", "X", "foo", "src/a.rs"]);
        assert_eq!((patterns, paths), (vec!["foo".to_string()], vec!["src/a.rs".to_string()]));
        assert_eq!(flags, vec!["-r", "X"]);
        let (patterns, _, flags) = extract_pattern_path(Engine::Rg, &["-E", "utf-8", "foo"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(flags, vec!["-E", "utf-8"]);
        // grep: -r is recursive and -T is a boolean.
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Grep, &["-rT", "foo", "src"]);
        assert_eq!((patterns, paths), (vec!["foo".to_string()], vec!["src".to_string()]));
        assert_eq!(flags, vec!["-rT"]);
    }
    ...
    fn test_parse_cluster_boolean_only() {
        ...
        assert_eq!(
            parse_cluster(Engine::Grep, "Rni"),
            ClusterResult::Boolean(Some("Rni".to_string()))
        );
        assert_eq!(
            parse_cluster(Engine::Grep, "n"),
            ClusterResult::Boolean(Some("n".to_string()))
        );
        assert_eq!(
            parse_cluster(Engine::Grep, "ni"),
    ...
    fn test_parse_cluster_e_no_inline() {
        ...
        assert_eq!(parse_cluster(Engine::Grep, "e"), vt(None, 'e', ""));
    }

    #[test]
    fn test_parse_cluster_e_inline_value() {
        // -ecarrot: inline="carrot" — no r/R stripping on the value bytes
        assert_eq!(parse_cluster(Engine::Grep, "ecarrot"), vt(None, 'e', "carrot"));
    }
    ...
    fn test_parse_cluster_e_inline_value_no_rstrip() {
        ...
        let ClusterResult::ValueTaking { inline, .. } = parse_cluster(Engine::Grep, "ecarrot") else {
    ...
    fn test_parse_cluster_ni_a() {
        ...
        assert_eq!(parse_cluster(Engine::Grep, "niA"), vt(Some("ni"), 'A', ""));
    }

    #[test]
    fn test_parse_cluster_ai_inline() {
        // -Ai: A value-taking, inline="i" (the 'i' is A's value, not a separate flag)
        assert_eq!(parse_cluster(Engine::Grep, "Ai"), vt(None, 'A', "i"));
    }
    ...
    fn test_extract_simple() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["foo", "src/"]);
        assert_eq!(patterns, vec!["foo"]);
    ...
    fn test_extract_with_bool_flag() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-i", "foo", "src/"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src/"]);
    ...
    fn test_extract_value_taking_flag() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-A", "2", "error", "src"]);
    ...
    fn test_extract_cluster_keeps_r() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-rn", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_cluster_ending_in_e() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-rne", "PATTERN", "src"]);
    ...
    fn test_extract_cluster_ending_in_value_flag() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-rA", "2", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_multi_path() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["TODO", "src", "tests"]);
    ...
    fn test_extract_e_flag() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-e", "fn run", "src"]);
    ...
    fn test_extract_multi_e() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-e", "foo", "-e", "bar", "src"]);
    ...
    fn test_extract_dashdash_boundary() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["--", "--version"]);
    ...
    fn test_extract_no_args() {
        let (patterns, paths, flags) = extract_pattern_path::<&str>(Engine::Grep, &[]);
    ...
    fn test_extract_default_path_empty() {
        ...
        let (patterns, paths, _) = extract_pattern_path(Engine::Grep, &["foo"]);
        assert_eq!(patterns, vec!["foo"]);
        assert!(paths.is_empty());
    }
    ...
    fn test_extract_ending_e() {
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Grep, &["-e", "foo", "-e", "bar", "src", "-e"]);
        assert_eq!(patterns, vec!["foo", "bar"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_inline_e_value() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-ecarrot", "file"]);
    ...
    fn test_extract_inline_e_value_no_rstrip() {
        ...
        let (patterns, _, _) = extract_pattern_path(Engine::Grep, &["-ecarrot", "file"]);
        assert_eq!(
            patterns,
            vec!["carrot"],
    ...
    fn test_extract_long_glob_value() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["compact", "sub", "--glob", "*.md"]);
    ...
    fn test_extract_long_max_count() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["--max-count", "1", "fn", "file"]);
    ...
    fn test_extract_short_max_depth() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["-d", "3", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_long_regexp() {
        // --regexp is the long form of -e; value goes to patterns
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["--regexp", "fn run", "src"]);
        assert_eq!(patterns, vec!["fn run"]);
        assert_eq!(paths, vec!["src"]);
        assert!(flags.is_empty());
    }
    ...
    fn test_extract_long_regexp_multi() {
        ...
        let (patterns, paths, _) = extract_pattern_path(Engine::Grep, &["--regexp", "foo", "-e", "bar", "src"]);
        assert_eq!(patterns, vec!["foo", "bar"]);
        assert_eq!(paths, vec!["src"]);
    }
    ...
    fn test_extract_long_ignore_file() {
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Grep, &["--ignore-file", ".myignore", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_long_engine() {
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["--engine", "pcre2", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_long_type_clear() {
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Grep, &["--type-clear", "rust", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_long_path_separator() {
        let (patterns, paths, flags) =
            extract_pattern_path(Engine::Grep, &["--path-separator", "/", "foo", "src"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
    ...
    fn test_extract_long_flag_inline_eq_passthrough() {
        ...
        let (patterns, paths, flags) = extract_pattern_path(Engine::Grep, &["foo", "src", "--glob=*.rs"]);
        assert_eq!(patterns, vec!["foo"]);
        assert_eq!(paths, vec!["src"]);
